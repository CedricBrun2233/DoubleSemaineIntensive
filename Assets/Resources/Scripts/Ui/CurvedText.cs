...
public class CurvedText : Text
{
    ...
    protected override void OnValidate ()
	{
		...
		if (curvature == 0 || Mathf.Abs (curvature) > 1) {
	...
	{	
		...
		for (int i = 0; i < stream.Count; i++) {
			...
			v.position = offset * radius * scaleFactor * curvature + offset * v.position.y;
			v.position += Vector3.down * radius * scaleFactor * curvature;
			if (curvature < 0)
				v.position = new Vector3 (v.position.x * -1, v.position.y, v.position.z);
